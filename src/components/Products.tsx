...
import { Leaf, Award, Compass, ArrowUpRight, Star, ChevronRight } from 'lucide-react';
...
export default function Products({ onQuoteRequest }: ProductsProps) {
  ...
  const products: ProductItem[] = [
    ...
    },
    {
      ...
      category: "fruits",
      desc: "Export-grade G9 Cavendish bananas directly sourced from Maharashtra farms. Packed in premium export cartons with phytosanitary certification for Middle East & Europe.",
      moq: "10 Metric Tons",
    ...
    },
    {
      ...
      title: "Fresh Papaya",
      category: "fruits",
      desc: "Premium export-quality papayas individually foam-net wrapped in export cartons. Inspected for size, weight, and Brix sugar levels. Ethylene-controlled ripening.",
      moq: "5 Metric Tons",
  ...
  return (
    <section id="products" className="py-16 md:py-24 bg-luxury-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 relative z-10">
        ...
        {/* ── ONION SPOTLIGHT BANNER ── */}
        <div className="mb-20 rounded-3xl overflow-hidden border border-luxury-gold/20 shadow-premium-hover relative">
          {/* Background image - raw onion warehouse */}
          <div className="absolute inset-0">
            <img
              src="images/onion_raw.jpeg"
              alt="Onion Sourcing"
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-0">
            {/* Left: Content */}
            <div className="p-6 sm:p-10 md:p-14 flex flex-col justify-center gap-6 text-left">
              ...
              <h3 className="text-3xl md:text-4xl font-luxury font-bold text-luxury-white leading-tight">
                Dehydrated Onion Products
                <span className="block text-luxury-gold text-xl md:text-2xl mt-2 font-light">Powder · Flakes · Minced · Granules</span>
              </h3>
              <p className="text-sm font-light text-neutral-300 leading-relaxed max-w-md">
                Nexorra Impex is a leading exporter of dehydrated onion products sourced directly from Maharashtra's prime onion belt. Our processing facilities maintain strict food-grade hygiene with APEDA-compliant packaging for global markets.
              </p>

              {/* Spec Grid */}
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3 mt-2">
                {onionSpecs.map((spec) => (
                  <div key={spec.label} className="bg-luxury-white/5 border border-luxury-gold/15 rounded-xl px-4 py-3 text-left">
              ...
              <div className="flex flex-wrap gap-3 mt-2">
                <button
                  onClick={() => onQuoteRequest('Onion Powder')}
                  className="flex items-center gap-2 px-6 py-3 bg-luxury-gold text-luxury-charcoal text-xs font-bold uppercase tracking-widest rounded-xl hover:bg-luxury-gold-light transition-all shadow-md"
                >
                  Request Onion Quote
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-6 py-3 border border-luxury-gold/30 text-luxury-gold text-xs font-bold uppercase tracking-widest rounded-xl hover:bg-luxury-gold/10 transition-all"
                >
            ...
            {/* Right: Two product image cards */}
            <div className="grid grid-cols-2 gap-0 min-h-[320px]">
              <div className="relative overflow-hidden group">
                <img
                  src="images/onion_powder.jpeg"
                  alt="Onion Powder"
                  className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"
                  onError={(e) => {
                    e.currentTarget.src = "https://images.unsplash.com/photo-1596040033229-a9821ebd058d?auto=format&fit=crop&q=80&w=400";
                  }}
                />
                <div className="absolute inset-0 bg-gradient-to-t from-luxury-charcoal/80 to-transparent" />
                <div className="absolute bottom-4 left-4">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-luxury-gold block">Grade A</span>
                  <span className="text-sm font-bold text-luxury-white font-luxury">Onion Powder</span>
                </div>
              </div>
              <div className="relative overflow-hidden group border-l border-luxury-gold/10">
                <img
                  src="images/onion_flakes.jpeg"
                  alt="Onion Flakes"
                  className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"
                  onError={(e) => {
                    e.currentTarget.src = "https://images.unsplash.com/photo-1596040033229-a9821ebd058d?auto=format&fit=crop&q=80&w=400";
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-luxury-charcoal/80 to-transparent" />
                <div className="absolute bottom-4 left-4">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-luxury-gold block">Export Ready</span>
                  <span className="text-sm font-bold text-luxury-white font-luxury">Onion Flakes</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <AnimatePresence mode="popLayout">
            {filteredProducts.map((prod) => (
              <motion.div
                ...
                whileHover={{ y: -8 }}
                className={`group flex flex-col bg-gradient-to-br from-luxury-cream to-luxury-white rounded-3xl border overflow-hidden shadow-premium hover:shadow-premium-hover transition-all duration-500 text-left shine-hover ${
                  prod.featured ? 'border-luxury-gold/30' : 'border-luxury-gold/15'
                }`}
              >
                {/* Image Showcase */}
